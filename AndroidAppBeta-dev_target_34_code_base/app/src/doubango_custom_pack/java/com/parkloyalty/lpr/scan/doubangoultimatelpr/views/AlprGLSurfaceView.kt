 ...
 */
package com.parkloyalty.lpr.scan.doubangoultimatelpr.views
 ...
 * GL surface view
 */
class AlprGLSurfaceView : GLSurfaceView, GLSurfaceView.Renderer {
    ...
    private val TAG = AlprGLSurfaceView::class.java.canonicalName
    private lateinit var vehicleBitmap: Bitmap
    ...
    private lateinit var flParentContainer : FrameLayout

    /**
     *
     * @param
     */
    fun setImage(image: Image, jpegOrientation: Int,  flParentContainer : FrameLayout) {
        this.flParentContainer = flParentContainer

        if (!isReady) {
            printLog(TAG, "Not ready")
            image.close()
            return
        }
        if (mImage != null) {
            printLog(TAG, "Already rendering previous image")
            image.close()
            return
        }
        ...
        if (mJpegOrientation != jpegOrientation) {
            printLog(TAG, "Orientation changed: $mJpegOrientation -> $jpegOrientation")
    ...
    override fun onDrawFrame(glUnused: GL10) {
        ...
        val imageWidth = mImage?.width.nullSafety()
        val imageHeight = mImage?.height.nullSafety()
        ...
        //TODO Camera
        //GLES20.glViewport(0, 0, imageWidth, this.flParentContainer.measuredHeight)
        //TODO Camera
        ...
        if (inBitmap == null || !inBitmap.isMutable
            || inBitmap.width != w || inBitmap.height != h
        ) {
...
//        inBitmap.compress(Bitmap.CompressFormat.JPEG, 80, bos)

        vehicleBitmap = inBitmap
...
//                    "printerscreenshots" + File.separator.toString() + "image"
//        )
//        dir_image.mkdirs()
//        try {
...
//        }
    ...
    fun getVehicleImage(): Bitmap {
        return vehicleBitmap
    }

    fun resetVehicleImage() {
    ...
    override fun onSurfaceCreated(glUnused: GL10, config: EGLConfig) {
        ...
        printLog(TAG, "OpenGL extensions=$extensions")
    ...
    private fun loadShader(shaderType: Int, source: String): Int {
        ...
        if (shader != 0) {
            ...
            if (compiled[0] == 0) {
                printLog(TAG, "Could not compile shader $shaderType:")
                printLog(TAG, GLES20.glGetShaderInfoLog(shader))
    ...
    private fun createProgram(vertexSource: String, fragmentSource: String): Int {
        ...
        if (program != 0) {
            ...
            if (linkStatus[0] != GLES20.GL_TRUE) {
                printLog(TAG, "Could not link program: ")
                printLog(TAG, GLES20.glGetProgramInfoLog(program))
