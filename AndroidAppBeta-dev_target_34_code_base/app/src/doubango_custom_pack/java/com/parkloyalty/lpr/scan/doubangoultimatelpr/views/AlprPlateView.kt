...
class AlprPlateView(private val mContext: Context, attrs: AttributeSet?) : View(
    mContext, attrs
) {
    ...
    override fun draw(canvas: Canvas) {
        super.draw(canvas)
        if (mImageSize == null) {
            printLog(TAG, "Not initialized yet")
            return
        }
        ...
        if (mPlates != null && !mPlates!!.isEmpty()) {
            for (plate in mPlates!!) {
                ...
                if (number != null && !number.isEmpty()) {
                    vehicleDetailListener?.getLicensePlateNumber(number.uppercase())
                ...
                if (plate.car != null) {
                    ...
                    if (car?.confidence.nullSafety() >= 80f) {
                        ...
                        vehicleDetailListener?.getVehicleColor(color?.uppercase())
                        ...
                        vehicleDetailListener?.getVehicleMakeBrand(make?.uppercase())
                        vehicleDetailListener?.getVehicleModel(model?.uppercase())
                        ...
                        vehicleDetailListener?.getVehicleBodyStyle(bodyStyle?.uppercase())
                ...
                if (plate.countries != null) {
                    ...
                    if (country.confidence >= LPCI_MIN_CONFIDENCE) {
                        //Start of State
                        vehicleDetailListener?.getLicensePlateState(country.state?.uppercase())
                        ...
                        vehicleDetailListener?.getLicensePlateCountry(countryString.uppercase())
                    }
                }
            }
        }
    ...
    companion object {
        val TAG = AlprPlateView::class.java.canonicalName
        const val LPCI_MIN_CONFIDENCE = 90f
