...
@HiltViewModel
class BootViewModel @Inject constructor(private val service: Service) : ViewModel() {
    private val disposables = CompositeDisposable()
    val response = MutableLiveData<ApiResponse?>()
    fun hitBootSubmitApi(bootRequest: BootRequest?) {
        disposables.add(
            service.executeBootAPI(bootRequest)
    ...
    val responseBootInstanceTicketAPI = MutableLiveData<ApiResponse?>()

    /**
     * This method is used to create a warning scofflaw ticket from Boot Activity
     */
    fun hitBootInstanceTicketAPI(bootMetadataRequest: BootMetadataRequest?) {
        disposables.add(
            service.executeBootInstanceTicketAPI(bootMetadataRequest)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe { d: Disposable? -> responseBootInstanceTicketAPI.setValue(loading()) }
                .subscribe(
                    { result: JsonNode? ->
                        responseBootInstanceTicketAPI.setValue(
                            success(
                                result!!
                            )
                        )
