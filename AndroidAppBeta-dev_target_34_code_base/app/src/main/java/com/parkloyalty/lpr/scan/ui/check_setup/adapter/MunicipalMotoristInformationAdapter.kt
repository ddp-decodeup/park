...
class MunicipalMotoristInformationAdapter
    (
    val mContext: Context,
    val mListData: List<VehicleListModel>?,
    val listItemSelectListener: ListItemSelectListener?
) : RecyclerView.Adapter<MunicipalMotoristInformationAdapter.ViewHolder>() {
            ...
            if (
                ...
                if (AppUtils.getSectionLineInPrintOut()) {
                    ...
                    AppUtils.printQueryStringBuilder = ZebraCommandPrintUtils.setXYforPrintSectionBox(
                        mListData!![0], mListData!![lastIndex], 1, "violation",
                        AppUtils.printQueryStringBuilder!!
                    )
                }
            }
...
//            listItem = layoutInflater.inflate(R.layout.content_double_text_print_ocean, parent, false)
...
//        listItem = layoutInflater.inflate(R.layout.content_double_text_print, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        try {
            if (mListData != null) {
                val violationDataItem = mListData[position]

                LogUtil.printLog("ViolationAdapter","${violationDataItem!!.offNameFirst}")
                LogUtil.printLog("ViolationAdapter","${violationDataItem!!.offTypeFirst}")
                LogUtil.printLog("ViolationAdapter","=======================")

                holder.tvField.text = violationDataItem!!.offNameFirst
                holder.tvFieldValue.text = violationDataItem!!.offTypeFirst
                if (violationDataItem!!.offNameFirst!=null && violationDataItem!!.offNameFirst!!.isEmpty()) {
                    holder.tvField.visibility = View.GONE
                }
                AppUtils.printQueryStringBuilder = ZebraCommandPrintUtils.setXYforPrint(mListData!!,position,PRINT_SECTION_VIOLATION,
                    AppUtils.printQueryStringBuilder!!)

                if (BuildConfig.FLAVOR.equals(Constants.FLAVOR_TYPE_PILOTPITTSBURGPA, ignoreCase = true)) {
                    if (violationDataItem!!.offNameFirst.equals("Location:")
                    ) {
    ...
    override fun getItemViewType(position: Int): Int {
        val violationDataItem = mListData?.get(position)

        return when (violationDataItem?.type) {
            ONECOLUMN -> {
                ONECOLUMN
            }
    ...
    inner class ViewHolder internal constructor(itemView: View) :
        ...
        init {
            ...
            //TODO Janak Having mismatch in different layout - main & duncan
