...
@HiltViewModel
class GeneticHitListViewModelMockup @Inject constructor(private val service: MockService) : ViewModel() {
    ...
    fun hitGetGeneticHitListApi(typeOfHit: String, lprNumber:String,page:String, limit:String) {
        disposables.add(
//            service.executeGetGeneticHitListAPI(typeOfHit, lprNumber, page, limit)
//                .subscribeOn(Schedulers.io())
//                .observeOn(AndroidSchedulers.mainThread())
//                .doOnSubscribe { ... }
//                .subscribe({ result -> success(result!!) }, { throwable -> error })
            ...
            service.executeGetGeneticHitListAPI( typeOfHit = typeOfHit,
                ...
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe { d: Disposable? -> response.value = loading() }
                .subscribe(
                    { result: JsonNode? ->
                    ...
                    { throwable: Throwable? ->
                        response.value = ApiResponse.error(
                            throwable!!
                        )
                    }
                ))
