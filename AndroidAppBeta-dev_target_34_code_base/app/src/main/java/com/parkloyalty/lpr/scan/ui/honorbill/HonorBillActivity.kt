...
//class OwnerBillActivity : BaseActivity() {}
...
import com.parkloyalty.lpr.scan.databinding.ActivityOwnerBillBinding
...
import com.parkloyalty.lpr.scan.ui.municipalcitation.MunicipalCitationPreviewActivity
import com.parkloyalty.lpr.scan.ui.honorbill.HonorBillConstructLayoutBuilder.CheckTypeOfField
...
/**
 * OwnerBillActivity: A refactored version of OwnerBillActivity,
 * adopting modern Android MVVM, View Binding, and Kotlin idiomatic practices.
 */
class HonorBillActivity : BaseActivity(), CustomDialogHelper {
    ...
    private var mCitationLayout: List<HonorBillCitationLayoutData>? = ArrayList()
...
//    private var mLastSecondCheckViewModel: LastSecondCheckViewModel? by viewModels()
...
//            mAutoComTextViewVinNum!!.setText(vinOCRText)
    ...
    private lateinit var binding: ActivityOwnerBillBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityOwnerBillBinding.inflate(layoutInflater)
    ...
    private fun findViewsByViewBinding() {
        linearLayoutVehicleDetails = binding.layoutContentOwnerBill.llVehicleDetails
        appCompatTextViewTicketNumberTitle =
            binding.layoutContentOwnerBill.txtTicketNumber
        linearLayoutLocationDetails =
            binding.layoutContentOwnerBill.layoutContentOwnerBillInternalNote.llLocationDetails
        layTicketDate = binding.layoutContentOwnerBill.layTicketDate
        layTicketNum = binding.layoutContentOwnerBill.layTicketNum
        linearLayoutOfficerDetails = binding.layoutContentOwnerBill.llOfficerDetails
        linearLayoutInternalNote =
            binding.layoutContentOwnerBill.layoutContentOwnerBillInternalNote.llMainInternalNote
        linearLayoutCitationDetails =
            binding.layoutContentOwnerBill.layoutContentOwnerBillInternalCameraView.llCitationDetails
        linearLayoutVoilationDetails =
            binding.layoutContentOwnerBill.layoutContentOwnerBillInternalNote.llVoilationDetails
        layCheckbox1 = binding.layoutContentOwnerBill.layCheckbox1
        layCheckbox2 = binding.layoutContentOwnerBill.layCheckbox2
        layCheckbox3 = binding.layoutContentOwnerBill.layCheckbox3
        tvVoilationDetailsFocus =
            binding.layoutContentOwnerBill.layoutContentOwnerBillInternalNote.tvVoilationDetailsFocus
        tvLocationDetailsFocus =
            binding.layoutContentOwnerBill.layoutContentOwnerBillInternalNote.tvLocationDetailsFocus
        viewLocationDetails =
            binding.layoutContentOwnerBill.layoutContentOwnerBillInternalNote.viewLocationDetails
        viewVoilationDetails =
            binding.layoutContentOwnerBill.layoutContentOwnerBillInternalNote.viewVoilationDetails
        mTvType1 = binding.layoutContentOwnerBill.tvType1
        mTvType2 = binding.layoutContentOwnerBill.tvType2
        mTvType3 = binding.layoutContentOwnerBill.tvType3
        mCheckboxType1 = binding.layoutContentOwnerBill.checkboxType1
        mCheckboxType2 = binding.layoutContentOwnerBill.checkboxType2
        mCheckboxType3 = binding.layoutContentOwnerBill.checkboxType3
        mTextViewSignName =
            binding.layoutContentOwnerBill.layoutContentOwnerBillInternalCameraView.txtPersonSignature
        mTvTicketNumber = binding.layoutContentOwnerBill.tvTicketNumber
        mTvTicketDate = binding.layoutContentOwnerBill.tvTicketDate
        mTvTicketDetailsHide = binding.layoutContentOwnerBill.tvTicketDetailsHide
        tvEnforcementTitle = binding.layoutContentOwnerBill.tvEnforcementTitle
        layTicketDetailsHide = binding.layoutContentOwnerBill.layTicketDetailsHide
        mlinearLayoutCheckBox = binding.layoutContentOwnerBill.linearLayoutCheckBox
        imageViewSignature =
            binding.layoutContentOwnerBill.layoutContentOwnerBillInternalCameraView.imgSignature
        appCompatTextViewTicketType = binding.layoutContentOwnerBill.textTicketType
        fabClearButton = binding.fabClear
        mViewPagerBanner =
            binding.layoutContentOwnerBill.layoutContentOwnerBillInternalCameraView.layoutContentBanner.pagerBanner
        pagerIndicator =
            binding.layoutContentOwnerBill.layoutContentOwnerBillInternalCameraView.layoutContentBanner.viewPagerCountDots
    }

    private fun setupClickListeners() {

        binding.layoutContentOwnerBill.layoutContentOwnerBillInternalCameraView.btnPreview.setOnClickListener {
            if (isCheckTicketAPIExecuted) {
                goToPreviewScreen()
            } else {
                showCustomAlertDialogWithPositiveButton(
                    this@HonorBillActivity,
        ...
        binding.layoutContentOwnerBill.ivCamera.setOnClickListener {
            requestPermission()
        }

        binding.layoutContentOwnerBill.layoutContentOwnerBillInternalCameraView.ivCameraBottom.setOnClickListener {
            requestPermission()
        }
        ...
        binding.layoutContentOwnerBill.layoutContentOwnerBillInternalCameraView.btnClear.setOnClickListener {
            ClearFileds()
        }
...
//                    mCheckboxType1.setChecked(false);
        ...
        binding.layoutContentOwnerBill.layoutDashboardHeader.imgBack.setOnClickListener {
            deleteImages()
        }
    ...
    private fun init() {
        ...
        addObservers()
        setToolbar()
//        val response = getMyDatabase()?.dbDAO?.getOwnerBillLayout()
//        sharedPreference.write(
...
//        {{local}}/analytics/mobile/pay_by_plate?zone=Indigo&Park&lp_number=VIM112
        ...
        if (intent != null) {
                ...
                if (intent.getStringExtra("from_scr") == "ticket_details") {
                    ...
                    mainScope.launch {
                        ...
                        if (mLockCitation.equals("lock", ignoreCase = true)) {
                            setLprLock(lockLprModel, this@HonorBillActivity, sharedPreference)
                            sharedPreference.write(
            ...
            if (!isIssue) {
                ...
                if (intent.hasExtra(INTENT_KEY_TIMING_IMAGES)) {
                    ...
                    LogUtil.printLog("==>TimingImageList:", ObjectMapperProvider.toJson(mTimingImages))
                } else {
    ...
    private fun callDownloadBitmapApi() {
        if (NetworkCheck.isInternetAvailable(this@HonorBillActivity)) {
...
//                   bannerList!!.removeAt(i)
    ...
    private fun requestPermission() {
        if (PermissionUtils.requestCameraAndStoragePermission(this@HonorBillActivity)) {
...
//                    Toast.makeText(requireContext(), "Set date: $year/$month/$dayOfMonth", Toast.LENGTH_LONG).show()
    ...
    private fun setDropdownState() {
        hideSoftKeyboard(this@HonorBillActivity)
                    ...
                    mAutoComTextViewState?.post {
                        ...
                        try {
                            ...
                            if (mAutoComTextViewState?.tag != null && mAutoComTextViewState?.tag == "listonly") {
                                setListOnly(
                                    this@HonorBillActivity, mAutoComTextViewState!!
                                )
                            }
                        } catch (e: java.lang.Exception) {
                            e.printStackTrace()
                    ...
                    mEtLocationRemarks?.post {
                        ...
                        try {
                            ...
                            if (mEtLocationRemarks?.tag != null && mEtLocationRemarks?.tag == "listonly" &&
                                ...
                                setListOnly(
                                    this@HonorBillActivity, mEtLocationRemarks!!
                                )
                            }
                        } catch (e: Exception) {
                            e.printStackTrace()
...
//                                                        parent.getItemAtPosition(position)
                            ...
                            if (mTvOfficerMake?.tag != null && mTvOfficerMake?.tag == "listonly") {
                                setListOnly(this@HonorBillActivity, mTvOfficerMake!!)
                            }
                        } catch (e: Exception) {
                        }
                    }
                        ...
                        mTvBeat?.post {
                            ...
                            try {
                                ...
                                if (mTvBeat!!.tag != null && mTvBeat!!.tag == "listonly") {
                                    setListOnly(this@HonorBillActivity, mTvBeat!!)
                                }
                            } catch (e: Exception) {
                                e.printStackTrace()
                            }
...
//            mApplicationList.add(DatasetResponse("RMVD"))
            mApplicationList.add(DatasetResponse("COVERED"))
            mApplicationList.add(DatasetResponse("UNKNOWN"))
            mApplicationList.add(DatasetResponse("UNREADABLE"))
        } else {
            mApplicationList.add(DatasetResponse("Not Visible"))
            mApplicationList.add(DatasetResponse("Partially Covered"))
        }
        val pos = 0
        if (mApplicationList != null && mApplicationList.size > 0) {
            val mDropdownList = arrayOfNulls<String>(mApplicationList.size)
                                ...
                                OnItemClickListener { parent, view, index, id ->
                                    ioScope.launch {
                                        ...
                                        if (mLockCitation.equals("lock", ignoreCase = true)) {
                                            setLprLock(
                                                lockLprModel,
                                                this@HonorBillActivity, sharedPreference
                                            )
                                            sharedPreference.write(
                                                SharedPrefKey.LOCKED_LPR_BOOL,
                                                true
                    ...
                    if (mCitationLayout!![iCit].component.equals(
                        ...
                        for (iOff in mCitationLayout!![iCit].fields!!.indices) {
                            if (mCitationLayout!![iCit].fields!![iOff].name.equals(
                                ...
                                mAutoComTextViewMotoristDateOfBirth?.hint =
                                    MOTORIST_DATE_OF_BIRTH_FORMAT
...
//                                    hideSoftKeyboard(this@OwnerBillActivity)
                            ...
                            if (mCitationLayout!![iCit].fields!![iOff].tag.equals("dropdown") ||
                                ...
                                if (mCitationLayout!![iCit].fields!![iOff].tag.equals("dropdown") ||
                                    ...
                                    mEtMarkTime?.setOnClickListener {
                                        AppUtils.hideSoftKeyboard(this@HonorBillActivity)
                                        openTimePicker(
                                            mEtMarkTime,
                                            this@HonorBillActivity,
                            ...
                            if (citationLayout.fields!![iOff].name.equals(
                                ...
                                if (citationLayout.fields!![iOff].isRequired.nullSafety()) {
                                    if (TextUtils.isEmpty(
                                        ...
                                        showKeyboard(this@HonorBillActivity)
                                        printToastMSGForErrorWarning(
                                            applicationContext,
...
//                                        mAutoComTextViewZone?.setError("")
                                ...
                                if (citationLayout.fields?.get(iOff)?.isRequired.nullSafety() || isPBCReqiredViolation) {
                                    if (TextUtils.isEmpty(
                                        ...
                                        AppUtils.showKeyboard(this@HonorBillActivity)
                                        LogUtil.printToastMSG(
                                            mContext,
                                            getString(R.string.val_msg_please_enter_zone)
                                        )
                    ...
                    if (citationLayout.component.equals(
                        ...
                        if (isMotoristDateOfBirthRequired && !isDateMatchingFormat(
                                mAutoComTextViewMotoristDateOfBirth?.text.toString().nullSafety(),
                                MOTORIST_DATE_OF_BIRTH_FORMAT
                            )
                        ) {
    ...
    private val uploadScannedImagesAPIResponseObserver = Observer<Any> {
        when (it) {
            is Resource.Error<*> -> {
                dismissLoader()
                showToast(
                    context = this@HonorBillActivity,
...
//                                    fname)
    ...
    private fun setSignatureView() {
        // custom dialog
        val dialog = Dialog(this@HonorBillActivity)
