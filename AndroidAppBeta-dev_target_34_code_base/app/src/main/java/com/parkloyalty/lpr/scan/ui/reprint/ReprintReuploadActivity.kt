package com.parkloyalty.lpr.scan.ui.reprint
...
//    class ReprintReuploadActivity : SwitchPrinterActivity(), PrintInterface, CustomDialogHelper {
    class ReprintReuploadActivity : BaseActivity(), PrintInterface, CustomDialogHelper {
...
//    private var mCommentsList = ArrayList<VehicleListModel>()
    ...
    private val bluetoothPermissionLauncher =
        registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { permissions ->
            ...
            if (granted) {
                ...
                BluetoothPermissionUtil.requestBluetoothSetting(this@ReprintReuploadActivity)
            }
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_reprint_reupload)
        ...
        if (LogUtil.getPrinterTypeForPrint() == PrinterType.STAR_PRINTER) {
            ...
            starPrinterUseCase?.initialize(this@ReprintReuploadActivity)
            ...
            xfPrinterUseCase?.initialize(this@ReprintReuploadActivity)
            ...
            zebraPrinterUseCase?.initialize(
                activity = this@ReprintReuploadActivity,
...
//        http://{{local}}/citations-issuer/citation_similarity_check
        if (NetworkCheck.isInternetAvailable(this@ReprintReuploadActivity)) {
            mFacsimileImagesViewModel!!.hitFacsimileImagesApi(ticketId)
        } else {
            LogUtil.printToastMSG(
                    this@ReprintReuploadActivity,
...
//                                mIssuranceModel = mDb!!.dbDAO!!.getCitationWithTicket(mCitationNumberId)
            ...
            Status.ERROR -> {
                dismissLoader()
                if (tag.equals(DynamicAPIPath.POST_ADD_IMAGE + "Image",ignoreCase = true)||
                    tag.equals(DynamicAPIPath.POST_IMAGE, ignoreCase = true)) {
                    AppUtils.showCustomAlertDialog(
                        mContext,
                        "Upload Failed",
                        getString(R.string.err_msg_something_went_wrong),
    ...
    fun setCitationDataOnObject(createTicketResponse : ResponseFacsimileImages)
    {
        ...
        mIssuranceModel!!.citationData!!.vehicle!!.decalYear =dataItem.vehicleDetails?.decalNumber.nullSafety()
...
//        mIssuranceModel!!.citationData!!.voilation!!.mLateFineDays =dataItem!!.violationDetails!!.lateFine
        ...
        mIssuranceModel?.citationData?.ticketDatePrint = AppUtils.dateFormateForFacsimil(dataItem.headerDetails?.timestamp)
...
//                        codeWriter.encode(ticketId, BarcodeFormat.CODE_128, 560, 44, hintMap)
                ...
                mainScope.launch {
                    linearLayoutCompatBarCode = this@ReprintReuploadActivity.layoutInflater.inflate(
...
//                if (BuildConfig.FLAVOR.equals(Constants.FLAVOR_TYPE_LAZ_CCP, ignoreCase = true)||BuildConfig.FLAVOR.equals(Constants.FLAVOR_TYPE_LAZ_KCMO_PRIVATE, ignoreCase = true)||
    ...
    private fun GenerateQRCodeInnovaPrint(ticketId: String, imageView: AppCompatImageView) {
        ...
        try {
            ...
            try {
                ...
                val bitmapQrCode =
                    QRCodeUtils.generateQRCodeForPrint(this@ReprintReuploadActivity, finalQR)
            ...
            printToastMSG(this@ReprintReuploadActivity, e.message)
        }
    }
...
//                        printcommand = AppUtils.setXYforPrint(citation,sequence,"citation",printcommand!!)
                ...
                if (mIssuranceModel?.citationData?.isStatus_ticket_date.nullSafety()) {
                    if (!TextUtils.isEmpty(mIssuranceModel?.citationData?.ticketDate.nullSafety())) {
                        try{
                            val (citation, citationTime) = CitationPrintSectionUtils.getTicketDate(this@ReprintReuploadActivity, mIssuranceModel)
...
//                                printcommand = AppUtils.setXYforPrint(citation,sequence,"citation",printcommand!!)
                ...
                if (mIssuranceModel?.citationData?.isStatus_ticket_type.nullSafety()) {
                    ...
                    val (isWarningSelected, citation) = CitationPrintSectionUtils.getTicketType(this@ReprintReuploadActivity,mIssuranceModel)
...
//                }
            ...
            try {
                val (orderNumber, location) = CitationPrintSectionUtils.getLocation(this@ReprintReuploadActivity, sharedPreference, mLocation, mIssuranceModel)
...
//                    val layoutSectionTitle = orderNumber.split("#").toTypedArray()[1]
            ...
            if (mIssuranceModel?.citationData?.location?.isStatus_CityZone.nullSafety()) {
                if (!TextUtils.isEmpty(mIssuranceModel?.citationData?.location?.cityZone)) {
                    val data = CitationPrintSectionUtils.getCityZone(this@ReprintReuploadActivity,mIssuranceModel)
                    val orderNumber =
            ...
            if (mIssuranceModel?.citationData?.location?.isStatus_PcbZone.nullSafety()) {
                if (!TextUtils.isEmpty(mIssuranceModel?.citationData?.location?.pcbZone.nullSafety())) {
                    val data = CitationPrintSectionUtils.getPCBZone(this@ReprintReuploadActivity,mIssuranceModel)
                    val orderNumber =
            ...
            try {
                if (mIssuranceModel?.citationData?.voilation?.isStatus_code.nullSafety()) {
                    if (!TextUtils.isEmpty(mIssuranceModel?.citationData?.voilation?.code.nullSafety())) {
                        val Vehdata = CitationPrintSectionUtils.getViolationCode(this@ReprintReuploadActivity,mIssuranceModel)
                        val orderNumber =
...
//                        printcommand = AppUtils.setXYforPrint(Vehdata,sequence,"violation",printcommand!!)
                ...
                if (mIssuranceModel?.citationData?.voilation?.isStatus_VioType.nullSafety()) {
                    if (!TextUtils.isEmpty(mIssuranceModel?.citationData?.voilation?.vioType.nullSafety())) {
                        val Vehdata = CitationPrintSectionUtils.getViolationVioType(this@ReprintReuploadActivity,mIssuranceModel)
                        val orderNumber =
                ...
                if (mIssuranceModel?.citationData?.voilation?.isStatus_VioTypeCode.nullSafety()) {
                    if (!TextUtils.isEmpty(mIssuranceModel?.citationData?.voilation?.vioTypeCode.nullSafety())) {
                        val Vehdata = CitationPrintSectionUtils.getViolationVioTypeCode(this@ReprintReuploadActivity,mIssuranceModel)
                        val orderNumber =
                ...
                if (mIssuranceModel?.citationData?.voilation?.isStatus_VioTypeDescription.nullSafety()) {
                    if (!TextUtils.isEmpty(mIssuranceModel?.citationData?.voilation?.vioTypeDescription.nullSafety())) {
                        val Vehdata = CitationPrintSectionUtils.getViolationVioTypeDescription(this@ReprintReuploadActivity,mIssuranceModel)
                        val orderNumber =
...
//            val itr = result.keys.iterator()
            ...
            if(BuildConfig.FLAVOR.equals(Constants.FLAVOR_TYPE_SANIBEL, true)||
                ...
                if (BuildConfig.FLAVOR.equals(
                    ...
                    vehicleListModelBarCode.offNameFirst = mCitationNumber
            ...
            try {
                ...
                e.printStackTrace()
                LogUtil.printToastMSG(this@ReprintReuploadActivity, getString(R.string.error_generate_facsimile_image))
            }
            try {
                Handler(Looper.getMainLooper()).postDelayed({
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                ...
                //QR CODE for ppa (TODO JANAK Need to check)
//                if (isQRCodePrintlayout) {
//                    appComTextViewPrintUrl.visibility = View.VISIBLE
//                    linearLayoutCompatPrintUrl.visibility = View.VISIBLE
...
//        }
    ...
    fun loadBitmapFromView(view: View?,isOCR: Boolean) {
        try {
            ...
            //Copied this conditions from LprPreviewActivity
            if (BuildConfig.FLAVOR.equals(Constants.FLAVOR_TYPE_DUNCAN, ignoreCase = true)||
                (BuildConfig.FLAVOR.equals(DuncanBrandingApp13())&&
                 !BuildConfig.FLAVOR.equals(Constants.FLAVOR_TYPE_NORWALK, ignoreCase = true)&&
                 !BuildConfig.FLAVOR.equals(Constants.FLAVOR_TYPE_KENOSHA, ignoreCase = true))) {
                ...
                BuildConfig.FLAVOR.equals(Constants.FLAVOR_TYPE_INDIAN_HARBOUR_BEACH, ignoreCase = true))
            {
                ...
                relativeLayoutPrintPhilaSection!!.visibility = View.INVISIBLE
                        ...
                        BuildConfig.FLAVOR.equals(Constants.FLAVOR_TYPE_LAZLB, ignoreCase = true)||
                        BuildConfig.FLAVOR.equals(Constants.FLAVOR_TYPE_KENOSHA, ignoreCase = true)) && !LogUtil.isEnableCommandBasedFacsimile)
            {
                relativeLayoutPrintAmountSection!!.visibility = View.VISIBLE
                relativeLayoutPrintStateSection!!.visibility = View.VISIBLE
                relativeLayoutPrintCitationSection!!.visibility = View.VISIBLE
            ...
            try {
                if (LogUtil.getPrinterTypeForPrint() == PrinterType.ZEBRA_PRINTER && AppUtils.isSiteSupportCommandPrinting(getSettingFileValuesForCMDPrinting())){
                    ...
                    AppUtils.printQueryStringBuilder = ZebraCommandPrintUtils.setAppLogoInCommandPrint(this@ReprintReuploadActivity,sharedPreference, AppUtils.printQueryStringBuilder)
                    ...
                    AppUtils.printQueryStringBuilder = ZebraCommandPrintUtils.setSignatureInCommandPrint(this@ReprintReuploadActivity,sharedPreference, AppUtils.printQueryStringBuilder)
                    ...
                    AppUtils.printQueryStringBuilder = ZebraCommandPrintUtils.setXYforPrintQRCode(this@ReprintReuploadActivity, AppUtils.printQueryStringBuilder)
                ...
                if (LogUtil.isEnableCommandBasedFacsimile){
                    LogUtil.printLog("==>Reprint","Calling If")
                    val bitmap = CanvasUtils.drawElementsToBitmapAutoSize(this@ReprintReuploadActivity, drawableElements)
                    ...
                    LogUtil.printLog("==>Reprint","Calling Else")

                    SaveImageMM(b, mCitationNumber + "_" + FILE_NAME_FACSIMILE_PRINT_BITMAP)
                }
            }catch (e:Exception){
                LogUtil.printLog("==>Reprint","Exception in command based facsimile: "+e.localizedMessage)
                e.printStackTrace()
...
//                }
                if (LogUtil.getPrinterTypeForPrint() == PrinterType.STAR_PRINTER) {
                    starPrinterUseCase?.mPrintFacsimileImage(
                        savePrintImagePath!!, this@ReprintReuploadActivity,
...
//                    getBitmapFromPath(savePrintImagePath!!, this@ReprintReuploadActivity,
//                        "previewScreen", "Valid", mCitationNumber,
//                        mFinalAmount,mIssuranceModel?.citationData?.vehicle!!.state,
//                        mIssuranceModel?.citationData?.vehicle!!.licensePlate,"",printcommand!!)
            ...
            LogUtil.printLog("==>Reprint","Exception Main : "+e.localizedMessage)
            e.printStackTrace()
        }
    }
